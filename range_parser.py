...
def range_parser(s):
    ...
    for x in arr:
        temp_list = x.split('-')
        if len(temp_list) == 1:
            result.append(int(temp_list[0]))
        elif len(temp_list) == 2:
            result.extend(list(range(int(temp_list[0]), int(temp_list[1]) + 1)))
        elif len(temp_list) == 3:
            result.extend(list(range(int(temp_list[0]), int(temp_list[1]) + 1, int(temp_list[2]))))
    return result
